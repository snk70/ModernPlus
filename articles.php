...
<META NAME="Description" CONTENT="مطالب و دانستنی ها, مقالات مربوط به آی تی و فناوری اطلاعات, دانستنی های روز فناوری اطلاعات, دانستنی های نرم افزار">
<META NAME="Keywords" CONTENT="مطالب و دانستنی های آی تی, دانستنی, کامپیوتر, آی تی, فناوری اطلاعات">

   

    <title>مطالب و دانستنی های فناوری اطلاعات - مدرن پلاس</title>
      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse navbar-right" style="direction:rtl;">
          <ul class="nav navbar-nav" >
                ...
                <li class="active"><a href="articles.php">مطالب</a></li>
...
{
				$rows2=mysql_query('SELECT * FROM `tbl_articles` ORDER BY `Row` DESC',$link);
				$lvl=0;
			   ...
			   echo('<img src="assets/img/portfolio/'.$r2["image_address"].'" class="img_prd">');
   			   echo('<div class="prt_prd_comments"><p class="pt2">'.$r["title"].'</p><p class="pt5">'.$r2["comment"].'</p></div>');
			   echo('</div></a>');
					}
				}
...
}else
{
			...
			$rw=mysql_query('SELECT * FROM tbl_articles ORDER BY `Row` DESC LIMIT 9');
		...
		$row=mysql_query('SELECT COUNT(Row) FROM `tbl_articles`',$link);
